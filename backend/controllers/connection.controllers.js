...
export const sendConnection = async (req, res) => {
  try {
    ...
    let user = await Connection.findById(sender);
    if (sender == id) {
      return res.staus(400).json({ message: "you can not send request yourself" });
    }
    if (user.connection.includes(id)) {
      return res.status(400).json({ message: "you have already connected" });
    }
    ...
    return res.status(500).json("sendConnection error");
  }
};
...
export const acceptConnection = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "acceptConnection error" });
  }
};
...
export const rejectConnection = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "rejectConnection error" });
  }
};
